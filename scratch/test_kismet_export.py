...
with open(log_path, "w") as f:
    try:
        # Create RenderTarget using KismetRenderingLibrary
        rt = unreal.KismetRenderingLibrary.create_render_target2d(None, 1280, 720, unreal.TextureRenderTargetFormat.RTF_RGBA8)
        ...
        if capture_actor:
            ...
            if comp:
                ...
                unreal.KismetRenderingLibrary.export_render_target(None, rt, target_dir, "ue_screenshot.png")
                f.write("Exported render target with KismetRenderingLibrary.export_render_target!\\n")
                
            unreal.EditorLevelLibrary.destroy_actor(capture_actor)
    except Exception as ex:
        f.write(f"Kismet export error: {ex}\\n")
"""

print("Executing Kismet RenderTarget Export via Python...")
res = send_request("execute_unreal_python", {"script": ue_script})
print("Result:", res)

# Check if file was created
if os.path.exists(shot_target):
    print("SUCCESS: Kismet export created ue_screenshot.png! Size:", os.path.getsize(shot_target))
else:
    print("FAILED: Kismet export file missing")
